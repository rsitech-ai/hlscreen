...
- Cargo workspace with `hls-core`, `hls-hyperliquid`, `hls-store`, `hls-features`, `hls-screen`, `hls-tui`, `hls-cli`, and `hls-server`.
...
- Health/telemetry state, deterministic reconnect simulation, TUI health rendering, `hls doctor --live`, and read-only local API response helpers.
...
The health and local API paths expose operational read-only state only; `hls server --print-health` prints the localhost JSON contract without starting a long-running API process.
