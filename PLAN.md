...
### Final Notes
- What changed: Partially completed US1 for confidence-aware rows and TUI presentation. Added `DataConfidenceSnapshot` to `FeatureSnapshot`, counted duplicate trade observations without replaying duplicate trades, added explicit `ConfidenceInputs` for reconnect gaps/parser drops/writer backlog, computed confidence in `hls-features`, added gap and sparse-trade fixtures, rendered confidence in the market-board header, rows, and selected-symbol detail pane, added degraded-confidence screenshot generation, and updated README/microstructure docs for the implemented confidence surface.
- Validation run: `cargo test -p hls-core --test market_state --test confidence_state`; `cargo test -p hls-features --test formulas`; `cargo test -p hls-tui --test main_table_golden --test confidence_pane`; `cargo test -p hls-cli --test live_mock --test health_commands`; `cargo test -p hls-screen --test dsl_evaluator --test presets`; `cargo test -p hls-server --test read_only_api`; `python3 scripts/generate-screenshots.py`; PNG previews with `rsvg-convert`; `cargo fmt --check`; `cargo clippy --workspace --all-targets --all-features -- -D warnings`; `cargo test --workspace --all-features`; `cargo build --workspace --all-features`; `git diff --check`; and a real public WebSocket smoke `./target/debug/hls live --symbols @107 --duration-secs 10 --refresh-secs 5 --tui` with 43 WebSocket messages, 70 market events, 0 reconnects, and 0 data gaps.
- Follow-ups: US1 is not complete. Remaining tasks are replay parity tests, CLI `--verify-parity`, persisted confidence metadata, parity drift detection, explicit live/replay confidence summary wiring, and final replay-parity documentation/task closeout.
