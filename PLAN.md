...
- Follow-ups: PR #4 (`actions/checkout@v7`) and PR #7 (`rusqlite@0.40.1`) were closed/superseded after the policy landed. PR #28 (`sha2@0.11.0`) exposed a real code compatibility issue in benchmark digest formatting and should be fixed by explicit byte-to-hex encoding, then rechecked against the Dependabot PR. GitHub branch-protection API returned `403` while the repo is private on the current account tier, so enforcement could not be API-verified until the repo is public or upgraded.
