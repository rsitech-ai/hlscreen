...
- Tradeoffs: `dist plan` was not run locally because the pinned `dist` CLI is not installed and installing global tooling would mutate the machine. CI and docs install pinned cargo-dist 0.32.0 before plan/build checks. Release publication is still unproven until the first reviewed `v*` tag workflow succeeds.
