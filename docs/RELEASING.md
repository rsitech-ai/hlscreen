...
| GitHub Release artifacts | Apple Silicon macOS archive and checksum published; the binary is Developer ID Application-signed (Team `2NY8A789TN`, hardened runtime); notarization status is stated per release in its notes | Other configured targets remain source-build only until native-runner package proof exists |
...
```bash
git tag -a v0.1.2 -m "hlscreen v0.1.2"
