# Documentation

Start with the public README, then use these files for deeper implementation and release context.

- [Architecture](architecture.md)
- [Production readiness](production-readiness.md)
- [Deployment](deployment.md)
- [Data format](data-format.md)
- [Development tooling provenance](DEVELOPMENT_TOOLING.md)
- [Feature definitions](feature-definitions.md)
- [Privacy](PRIVACY.md)
- [Threat model](THREAT_MODEL.md)
- [Roadmap](ROADMAP.md)
- [Release checklist](RELEASING.md)
- [Maintainers](../MAINTAINERS.md)
- [v0.1.0 release notes](releases/v0.1.0.md)
- [Open source checklist](OPEN_SOURCE_CHECKLIST.md)
- [Latest supervised all-symbol proof (2026-07-20)](evidence/soak/sota-allpairs-20260720-15m.json)
- [Production-readiness live refresh (2026-07-08)](reports/2026-07-08-production-readiness-live-refresh.md)
- [Latest end-to-end release audit](reports/2026-07-10-end-to-end-release-audit.md)
- [All-data E2E audit](reports/2026-07-08-all-data-e2e-audit.md)
- [Pre-merge audit](reports/2026-07-08-pre-merge-audit.md)

The active Spec Kit package lives under
`../specs/004-advanced-tui-workstation/`. The original v1 package under
`../specs/001-hyperliquid-spot-screener/` and the completed
`../specs/002-microstructure-workstation/` package are retained as historical
design records.
