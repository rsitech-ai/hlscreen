...
#[test]
fn standard_wide_header_renders_selected_quote_rail() {
    let model = RatatuiFrameModel::new(
        fixture_snapshots(),
        "READ-ONLY Hyperliquid spot live screen",
        ScreenRequest::default(),
    ...
    .expect("renders standard-wide selected quote rail");

    assert!(rendered.contains("layout wide 160x48"));
    assert!(rendered.contains("SELECTED QUOTE"));
    assert!(rendered.contains("HYPE/USDC"));
    assert!(rendered.contains("bid share"));
    assert!(rendered.contains("ask share"));
...
#[test]
fn expanded_book_renders_queue_terminal() {
    let snapshots = fixture_snapshots();
    let mut state = WorkstationUiState::default();
    state.apply(
        WorkstationAction::FocusPane(WorkstationPane::Book),
    ...
    .expect("renders expanded book queue terminal");

    assert!(rendered.contains("EXPANDED book"));
    assert!(rendered.contains("QUEUE TERMINAL"));
    assert!(rendered.contains("HYPE/USDC public top-book model"));
    assert!(rendered.contains("passive wall"));
    assert!(rendered.contains("aggressive OFI"));
    assert!(rendered.contains("friction gate"));
    assert!(rendered.contains("resilience"));
    assert!(rendered.contains("read-only BBO model"));
    assert!(rendered.contains("no wallet"));
    assert!(rendered.contains("no orders"));
}
...
#[test]
fn expanded_tape_renders_print_terminal() {
    let snapshots = fixture_snapshots();
    let mut state = WorkstationUiState::default();
    state.apply(
        WorkstationAction::FocusPane(WorkstationPane::Tape),
    ...
    .expect("renders expanded print terminal");

    assert!(rendered.contains("EXPANDED tape"));
    assert!(rendered.contains("PRINT TERMINAL"));
    assert!(rendered.contains("HYPE/USDC public trade tape"));
    assert!(rendered.contains("burst"));
    assert!(rendered.contains("largest print"));
    assert!(rendered.contains("side pressure"));
    assert!(rendered.contains("buy notional"));
    assert!(rendered.contains("sell notional"));
    assert!(rendered.contains("public trades only"));
    assert!(rendered.contains("no fills"));
    assert!(rendered.contains("no orders"));
}
...
fn cockpit_reflects_keyboard_view_pause_density_and_help_state() {
    ...
    assert!(rendered.contains("active pane click zoom"));
...
fn help_overlay_color_mode_renders_operator_keyboard_map() {
    ...
    assert!(colored.contains("active pane click zoom"));
